...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'export-b2c-promotion-gwp' });
const workerHelpers = require('../../helpers/workerHelper');
const fieldName = {
	'GWP ID': 'gwp_id',
	'GWP Name': 'gwp_name',
	'GWP Product': 'gwp_product',
	'Unique Order': 'unique_order',
	'Total Paid': 'total_paid',
	Sales: 'sales',
	Quota: 'total_quota',
	'Redemption Rate': 'redemption_rate_percentage',
	'Free Products Type': 'free_products_type',
	'Period Start': 'period_start',
	'Period End': 'period_end',
	'T&C': 'voucher_info_tnc',
	'Period Redeemed': 'period_date',
};

class ExportB2CPromotionGwpV2 extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-b2c-promotion-gwp');
	}
	getTotalCount() {
		return Q.try(() => {
			logger.info(
	...
	processBatch(limit, offset) {
		...
		return Q.try(() => {
			...
			const query = `SELECT * FROM ${this.options.document.tmp_table} ORDER BY gwp_id DESC LIMIT ${limit} OFFSET ${offset}`;
			...
			.then((result) => {
				...
				for (const row of rows) {
					const data = Object.entries(fieldName).reduce((acc, el) => {
						if (el[1] === 'redemption_rate_percentage') {
							acc[el[0]] = parseFloat(row[el[1]]).toFixed(2) + '%';
						} else if (el[1] === 'voucher_info_tnc') {
							acc[el[0]] = CommonHelper.slugify(row[el[1]]);
						} else if (['period_start', 'period_end'].includes(el[1])) {
...
module.exports = async (message) => {
	...
	logger.info(`[${context?.user?.name} - ${context?.user?.email}] Start jarvis ${isV2} b2c-promotion-gwp`);
	...
	const task = new ExportB2CPromotionGwpV2({
		limit: 500,
		offset: 0,
	...
	try {
		...
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: 'Export B2C Promotion GWP ID Report',
		});
