...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'export-omnichannel-all-order-with-item' });
const workerHelpers = require('../../helpers/workerHelper');

class OmnichannelAllOrderWithItem extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-omnichannel-all-order-with-item');
	}
	getTotalCount() {
		return Q.try(() => {
...
module.exports = async (message) => {
	...
	logger.info(
		`[${context?.user?.name} - ${context?.user?.email}] Start jarvis ${isV2} omnichannel-all-order-with-item`,
	);
	...
	let fieldName = {
		...
		'Default Category': 'category_defaul',
		Qty: 'qty',
	...
	if (isV2) {
		fieldName = {
			...
			'Delivered Date Ori': 'delivered_date',
			'Id Product': 'id_product',
			'Product Reference': 'product_reference',
			'Product Name': 'product_name',
			'Id Product Attribute': 'id_product_attribute',
			'Product Attribute': 'product_combination',
			Brand: 'brand',
			'Product Type': 'product_purchase_type',
			'Product Classification': 'product_classification',
	...
	const task = new OmnichannelAllOrderWithItem({
		limit: 500,
		offset: 0,
	...
	try {
		...
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: `Export ${isV2} Omnichannel All Order With Item`,
		});
	} catch (error) {
