...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'export-store-summary-pov' });
const workerHelpers = require('../../helpers/workerHelper');

class StoreSummaryPovCsv extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-store-summary-pov');
	}
	getTotalCount() {
		return Q.try(() => {
...
module.exports = async (message) => {
	...
	logger.info(`[${context?.user?.name} - ${context?.user?.email}] Start jarvis export-store-summary-pov`);

	const fieldName = {
		'STORE NAME': 'store_name',
		'USER REGISTERED': 'user_registered',
		'USER SHOPPED': 'user_shopped',
		'USER CONVERSION': 'user_conversion',
	};
	...
	const task = new StoreSummaryPovCsv({
		limit: 500,
		offset: 0,
	...
	try {
		...
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: 'Export Store Summary POV',
		});
	} catch (error) {
