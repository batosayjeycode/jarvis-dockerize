...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'export-excel-product-sourcing-rules-logs' });
const workerHelpers = require('../../helpers/workerHelper');
const writeXlsxFile = require('write-excel-file/node');

class ExcelProductSourcingRulesLogs extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-excel-product-sourcing-rules-logs');
	}
	getTotalCount() {
		return Q.try(() => {
		...
		return Q.try(() => {
			...
			for (const log of logs) {
				...
				const data = [
					...
					{
						value: `ID: ${log?.data?.id}, Data Type: ${log?.data?.data_type}, Name: ${log?.data?.name}, Cerebro Indo Company ID: ${log?.data?.source_id}, Cerebro VN Company ID: ${log?.data?.source_vn}`,
						borderColor: '#000000',
						align: 'left',
					},
				];
...
module.exports = async (message) => {
	...
	logger.info(`[${context?.user?.name} - ${context?.user?.email}] Start jarvis excel-product-sourcing-rules-logs`);
	...
	const task = new ExcelProductSourcingRulesLogs({
		limit: 500,
		offset: 0,
		filter: filter,
		context,
	...
	try {
		...
		await writeXlsxFile(data, {
			columns,
			filePath: outputFile,
			sheet: 'Product Sourcing Rule Log',
		});
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: 'Jarvis Export Excel Product Sourcing Rule Log',
