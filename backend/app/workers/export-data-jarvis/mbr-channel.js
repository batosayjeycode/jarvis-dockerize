...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'export-all-sociolla-mbr-channel' });
...
const fieldName = {
	...
	'Net Revenue Vs Budget': 'net_revenue_vs_budget',
...
class ExportAllSociollaMbrChannel extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-all-sociolla-mbr-channel');
	}
	getTotalCount() {
		return Q.try(() => {
...
module.exports = async (message) => {
	logger.info('Start jarvis all-sociolla-mbr-channel');
	...
	const task = new ExportAllSociollaMbrChannel({
		limit: 500,
		offset: 0,
	...
	try {
		...
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: 'Export Monthly Business Report - Channel',
		});
	} catch (error) {
