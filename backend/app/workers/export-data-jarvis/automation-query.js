...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'export-automation-query-csv' });
...
class AutomationQueryCsv extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-automation-query-csv');
	}
	getTotalCount() {
		return Q.try(() => {
...
module.exports = async (message) => {
	logger.info('Start jarvis export-automation-query-csv');
	...
	const task = new AutomationQueryCsv({
		limit: 500,
		offset: 0,
	...
	try {
		...
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: 'Export Automation Query',
		});
	} catch (error) {
