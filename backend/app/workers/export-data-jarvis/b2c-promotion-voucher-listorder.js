...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'b2c-promotion-voucher-listorder' });
const workerHelpers = require('../../helpers/workerHelper');

class ExportB2CPromotionVoucherListorder extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-b2c-promotion-voucher-listorder');
	}
	getTotalCount() {
		return Q.try(() => {
			...
			.then((result) => {
				...
				for (const row of rows) {
					self.options.csv.push({
						...
						Name: row.fullname,
						Email: this.options.document.is_email ? row.email : '-',
						Qty: row.qty,
						Nominal: row.nominal,
						Value: row.value,
					});
...
module.exports = async (message) => {
	...
	logger.info(`[${context?.user?.name} - ${context?.user?.email}] Start jarvis ${isV2} b2c-promotion-listorder`);

	const outputFile = file_name;
	const fields = ['Reference', 'Order Date', 'Name', 'Email', 'Qty', 'Nominal', 'Value'];
	const json2csv = new Transform({ fields }, { objectMode: true });
	const output = fs.createWriteStream(outputFile, { flags: 'a' });
	const input = new Readable({ objectMode: true });
	input._read = () => {};
	input.pipe(json2csv).pipe(output);
	const task = new ExportB2CPromotionVoucherListorder({
		limit: 500,
		offset: 0,
	...
	try {
		...
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: 'Export B2C Promotion Voucher Usage Listorder',
		});
		if (document?.isV2) {
