...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'export-financial-report-net-revenue-vn' });
...
module.exports = async (message) => {
	...
	logger.info(
		`[${context?.user?.name} - ${context?.user?.email}] Start jarvis export-financial-report-net-revenue-vn`,
	);

	const connEmailLogs = clientMongoDB.db(process.env.MS_SOCIOLLA_MONGODB).collection('email_logs');
	const title = document?.is_task_scheduler ? 'Net Revenue YTD VN' : 'Export Financial Report Net Revenue VN';
	...
	return Q.try(() => {
		...
		const emptyObjRowTop = generateEmptyObj(41, { topBorderColor: '#000000' });
		const emptyObjRows26 = generateEmptyObj(39);
		const emptyObjRowBot = generateEmptyObj(35, { bottomBorderColor: '#000000' });
		const emptyObjRowsWidth = generateEmptyObj(36, { width: 15 });
		...
		fields_default.forEach((f, i) => {
			...
			if (['Number', 'String'].includes(f.key)) {
				objects.forEach((row, j) => {
					rightBorder = i >= 23 && i < 40 && j === 39 ? { rightBorderColor: '#000000' } : {};
					const fontWeight = j >= 37 ? { fontWeight: 'bold' } : {};
					if (row?.val === 'gray') {
						ROWS.push({ backgroundColor: '#e7e6e6', ...rightBorder });
					} else if (row[f.column]) {
						if ([2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35, 39].includes(j) || f.key === 'String') {
