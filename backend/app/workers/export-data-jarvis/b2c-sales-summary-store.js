...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'b2c-sales-summary-store' });
const { Readable } = require('stream');
const workerHelpers = require('../../helpers/workerHelper');

class ExportB2CSalesSummaryStore extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-b2c-sales-summary-store');
	}
	getTotalCount() {
		return Q.try(() => {
...
const doGenerateFields = (document) => {
	...
	if (document?.isV2) {
		...
		fields.alias = 'Store Alias';
	} else {
		fields.store_name = 'Store Name';
		fields.store_alias = 'Store Alias';
	}
	return fields;
};
...
module.exports = async (message) => {
	...
	logger.info(`[${context?.user?.name} - ${context?.user?.email}] Start jarvis b2c-sales-summary-store`);
	...
	const task = new ExportB2CSalesSummaryStore({
		limit: 500,
		offset: 0,
	...
	try {
		...
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: `Export B2C ${isV2} Sales Summary By Store`,
		});
		if (document?.isV2) {
