...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'export-top-reviewer-vn' });
const workerHelpers = require('../../helpers/workerHelper');
const fieldName = {
	Username: 'username',
	'Total Review': 'total_review',
	'Trusted Review': 'trusted_review',
	'General Review': 'general_review',
};

class TopReviewerVn extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-top-reviewer-vn');
	}
	getTotalCount() {
		return Q.try(() => {
...
module.exports = async (message) => {
	...
	logger.info(`[${context?.user?.name} - ${context?.user?.email}] Start jarvis top-reviewer-vn`);
	...
	try {
		...
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: 'Export Top Reviewer VN Report',
		});
	} catch (error) {
