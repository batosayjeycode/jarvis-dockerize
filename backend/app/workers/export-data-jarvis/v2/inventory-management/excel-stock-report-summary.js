...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'v2-inventory-stock-report-summary' });
...
class ExportExcelStockReportSummary extends AbstractBatchTask {
	constructor(options) {
		super(options, 'v2-inventory-stock-report-summary');
	}
	getTotalCount() {
...
module.exports = async (message) => {
	...
	logger.info(`[${context?.user?.name}] Start v2-inventory-stock-report-summary`);
	...
	ExcelTransform.prototype._flush = async function (callback) {
		...
		workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: 'Jarvis : Export Platform Stock Summary',
	...
	const worksheet = workbook.addWorksheet('Platform Stock Summary');
	...
	input.push({ [firstKey]: 'Platform Stock Summary' });
	...
	const task = new ExportExcelStockReportSummary({
		limit: 500,
		offset: 0,
