				...
				for (const row of rows) {
					...
					for (const val of Object.entries(this.options.fieldName)) {
						...
						if (numberFields.includes(val[1])) {
							obj = { ...obj, value: value ? parseFloat(value) : 0, type: Number, format: '#,##0.00' };
...
module.exports = async (message) => {
	...
	const fieldName = {
		...
		'Sales Order Reference': 'sales_order_reference',
		'Gross | Qty': 'gross_qty',
		...(document?.is_valid_details ? valid_fields : {}),
		...(document?.is_net_details ? net_fields : {}),
	};

	const sheetTitle = document?.is_net_details ? 'Related Net' : 'Related Valid';
	...
	const columns = Array(Object.keys(fieldName).length).fill({ width: 20 });
	...
	try {
		...
		grand_total['document_reference'] = 'Grand Total';
		grand_total['type'] = '';
