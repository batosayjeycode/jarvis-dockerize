...
const logger = require('sociolla-core/lib/logger').getInstance({
	worker: 'export-v2-excel-download-sinclair-sssg-rules',
});
...
class ExportDownloadSssgRules extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-v2-excel-download-sinclair-sssg-rules');
	}
	getTotalCount() {
		return Q.try(() => {
...
module.exports = async (message) => {
	...
	logger.info(
		`[${context?.user?.name} - ${context?.user?.email}] Start jarvis export-v2-excel-download-sinclair-sssg-rules`,
	);
	...
	const fieldName = {
		...
		Active: 'is_active',
		Year: 'effective_year',
		'Comp. Store Status': 'is_comp_store',
	};
	...
	const task = new ExportDownloadSssgRules({
		limit: 500,
		offset: 0,
