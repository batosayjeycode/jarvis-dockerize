...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'export-financial-report-net-revenue' });
...
module.exports = async (message) => {
	...
	logger.info(`[${context?.user?.name} - ${context?.user?.email}] Start jarvis export-financial-report-net-revenue`);
		...
		fields_default.forEach((f, i) => {
			...
			if (['Number', 'String'].includes(f.key)) {
				objects.forEach((row, j) => {
					rightBorder = i >= 23 && i < 40 && j === 52 ? { rightBorderColor: '#000000' } : {};
