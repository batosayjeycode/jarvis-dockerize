...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'b2c-sales-summary' });
const { Readable } = require('stream');
const workerHelpers = require('../../helpers/workerHelper');

class ExportB2CSalesSummary extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-b2c-sales-summary');
	}
	getTotalCount() {
		return Q.try(() => {
...
module.exports = async (message) => {
	...
	logger.info(`[${context?.user?.name} - ${context?.user?.email}] Start jarvis ${isV2} b2c-sales-summary`);
	...
	const task = new ExportB2CSalesSummary({
		limit: 500,
		offset: 0,
	...
	try {
		...
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: `Export B2C ${isV2} Sales Summary`,
		});
		if (document?.isV2) {
