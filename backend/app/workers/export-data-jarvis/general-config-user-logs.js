...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'export-general-config-user-logs' });
const workerHelpers = require('../../helpers/workerHelper');

class ExportGeneralConfigUserLogs extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-general-config-user-logs');
	}
	getTotalCount() {
		return Q.try(() => {
		...
		return Q.try(() => {
			...
			for (const log of logs) {
				self.options.csv.push({
					...
					action: log.action,
					url_accessed: log.url,
					csv_download_size: log.filesize,
				});
			}
		});
	}
...
module.exports = async (message) => {
	...
	logger.info(`[${context?.user?.name} - ${context?.user?.email}] Start jarvis general-config-user-logs`);
	...
	const fields = ['timestamp', 'user_name', 'user_role', 'action', 'url_accessed', 'csv_download_size'];
	const json2csv = new Transform({ fields }, { objectMode: true });
	const output = fs.createWriteStream(outputFile, { flags: 'a' });
	const input = new Readable({ objectMode: true });
	input._read = () => {};
	input.pipe(json2csv).pipe(output);
	const task = new ExportGeneralConfigUserLogs({
		limit: 500,
		offset: 0,
		filter: filter,
		context,
	...
	try {
		...
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: 'Jarvis Export General Config User Logs',
		});
	} catch (error) {
