...
const logger = require('sociolla-core/lib/logger').getInstance({
	worker: 'export-b2c-analysistools-cancel-refund-detail-item',
});
const workerHelpers = require('../../helpers/workerHelper');

class ExportB2CAnalysistoolsCancelRefundDetailItem extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-b2c-analysistools-cancel-refund-detail-item');
	}
	getTotalCount() {
		return Q.try(() => {
...
module.exports = async (message) => {
	...
	logger.info(
		`[${context?.user?.name} - ${context?.user?.email}] Start jarvis ${isV2} b2c-analysistools-cancel-refund-detail-item`,
	);
	...
	if (isV2) {
		...
		const data = await client.connectNewDwh.query(queryRow);
		document.total = data?.rowCount || 0;
		logger.info(`[${context?.user?.name} - ${context?.user?.email}] TABLE ${document.tmp_table} CREATED`);
	}

	const task = new ExportB2CAnalysistoolsCancelRefundDetailItem({
		limit: 500,
		offset: 0,
	...
	try {
		...
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: `Export B2C ${isV2} Analysis Tools Cancel Refund Detail Item`,
		});
		if (isV2) {
