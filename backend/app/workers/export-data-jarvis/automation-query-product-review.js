...
const logger = require('sociolla-core/lib/logger').getInstance({
	worker: 'export-automation-query-product-review-csv',
});
...
class AutomationQueryProductReviewCsv extends AbstractBatchTask {
	constructor(options) {
		super(options, 'export-automation-query-product-review-csv');
	}
	getTotalCount() {
		return Q.try(() => {
...
module.exports = async (message) => {
	logger.info('Start jarvis export-automation-query-product-review-csv');
	...
	const task = new AutomationQueryProductReviewCsv({
		limit: 500,
		offset: 0,
	...
	try {
		...
		await workerHelpers.sendmail({
			...
			clientJarvis,
			criteria,
			subject: 'Export Automation Query Product Review',
		});
	} catch (error) {
