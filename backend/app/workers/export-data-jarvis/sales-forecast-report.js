...
const logger = require('sociolla-core/lib/logger').getInstance({ worker: 'export-sales-forecast-report' });
...
class SalesForecastReport extends AbstractBatchTask {
	constructor(options) {
		super(options, 'sales-forecast-report');
	}
	getTotalCount() {
		return Q.try(() => {
...
module.exports = async (message) => {
	...
	logger.info(`[${context?.user?.name} - ${context?.user?.email}] Start jarvis sales-forecast-report`);
	...
	const task = new SalesForecastReport({
		limit: 500,
		offset: 0,
